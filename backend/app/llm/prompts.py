...
VERDICT_SYSTEM = (
    ...
    "that Hermes cannot rebut with facts, lean REJECT. Treat likely vendor "
    "impersonation/typosquatting as a specific risk that should generally lead "
    "to REJECT (not escalate) when identity evidence is missing. Internally "
    "arbitrate by "
...
def build_verdict_messages(
    ...
    user_content = (
        ...
        "- If there are credible impersonation signals (e.g., brand-like spelling substitutions such as O->0) and vendor identity remains unverified, prefer REJECT over ESCALATE.\n"
        "- Use ESCALATE only when additional human-only evidence could realistically resolve the case; do not escalate when current evidence already indicates likely fraud.\n"
